...
resource "aembit_client_workload" "this" {
  ...
  description = "${var.client_workload_name} Client Workload"
  is_active   = var.is_active
  identities  = var.client_workload_identifiers
  tags        = var.tags

  standalone_certificate_authority = var.standalone_certificate_authority
}

resource "aembit_trust_provider" "this" {
  for_each = var.create_trust_providers == true && var.trust_providers != null ? var.trust_providers : {}
  ...
  certificate_signed_attestation = each.value["type"] == "certificate_signed_attestation" ? each.value["certificate_signed_attestation"] : null
  ...
  oidc_id_token                  = each.value["type"] == "oidc_id_token" ? each.value["oidc_id_token"] : null
...
resource "aembit_credential_provider" "this" {
  for_each = var.create_credential_providers == true && var.credential_providers != null ? var.credential_providers : {}
  ...
  azure_key_vault_value         = each.value["type"] == "azure_key_vault_value" ? each.value["azure_key_vault_value"] : null
  ...
  aws_secrets_manager_value     = each.value["type"] == "aws_secrets_manager_value" ? each.value["aws_secrets_manager_value"] : null
  jwt_svid_token                = each.value["type"] == "jwt_svid_token" ? each.value["jwt_svid_token"] : null
  oidc_id_token                 = each.value["type"] == "oidc_id_token" ? each.value["oidc_id_token"] : null
...
resource "aembit_access_policy" "this" {
  ...
  trust_providers   = var.create_trust_providers ? [for v in aembit_trust_provider.this : v["id"]] : compact([for v in data.aembit_trust_providers.this.trust_providers : v["name"] == var.trust_provider_name ? v["id"] : ""])
  ...
  credential_provider = each.value["credential_provider_name"] != null ? try(
    aembit_credential_provider.this[each.value["credential_provider_name"]].id,
    compact([for v in data.aembit_credential_providers.this.credential_providers : v["name"] == each.value["credential_provider_name"] ? v["id"] : ""])[0]
  ) : null
  server_workload = each.value["server_workload_id"] != null ? each.value["server_workload_id"] : compact([for v in data.aembit_server_workloads.this.server_workloads : v["name"] == each.value["server_workload_name"] ? v["id"] : ""])[0]
  is_active       = try(each.value["is_active"], true)

  credential_providers = each.value["credential_providers"] != null ? [
    for cp in each.value["credential_providers"] : {
      credential_provider_id = try(
        aembit_credential_provider.this[cp["credential_provider_name"]].id,
        compact([for v in data.aembit_credential_providers.this.credential_providers : v["name"] == cp["credential_provider_name"] ? v["id"] : ""])[0]
      )
      mapping_type         = cp["mapping_type"]
      access_key_id        = cp["access_key_id"]
      account_name         = cp["account_name"]
      header_name          = cp["header_name"]
      header_value         = cp["header_value"]
      httpbody_field_path  = cp["httpbody_field_path"]
      httpbody_field_value = cp["httpbody_field_value"]
    }
  ] : null
}
